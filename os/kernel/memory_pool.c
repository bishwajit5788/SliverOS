...
/* Pool class configuration */
static const size_t s_block_sizes[MK_POOL_CLASSES_COUNT] = { 16U, 32U, 64U, 128U, 256U };
#define MK_POOL_0_COUNT 32U /* 16-byte blocks */
#define MK_POOL_1_COUNT 32U /* 32-byte blocks */
#define MK_POOL_2_COUNT 32U /* 64-byte blocks */
#define MK_POOL_3_COUNT 16U /* 128-byte blocks */
#define MK_POOL_4_COUNT 8U  /* 256-byte blocks */
...
/* Static internal SRAM buffers for each class */
static uint8_t s_pool_buf_0[MK_POOL_0_COUNT * (sizeof(mk_pool_node_t) + 16U)] __attribute__((aligned(8)));
static uint8_t s_pool_buf_1[MK_POOL_1_COUNT * (sizeof(mk_pool_node_t) + 32U)] __attribute__((aligned(8)));
static uint8_t s_pool_buf_2[MK_POOL_2_COUNT * (sizeof(mk_pool_node_t) + 64U)] __attribute__((aligned(8)));
static uint8_t s_pool_buf_3[MK_POOL_3_COUNT * (sizeof(mk_pool_node_t) + 128U)] __attribute__((aligned(8)));
static uint8_t s_pool_buf_4[MK_POOL_4_COUNT * (sizeof(mk_pool_node_t) + 256U)] __attribute__((aligned(8)));
...
{
    if (!s_initialized) {
        if (mk_pool_init() != MK_STATUS_OK) return NULL;
    }

    if (size == 0 || size > s_block_sizes[MK_POOL_CLASSES_COUNT - 1]) {
        return NULL;
    }

    /* Find smallest matching class */
    int target_class = -1;
    for (uint8_t c = 0; c < MK_POOL_CLASSES_COUNT; c++) {
        if (size <= s_block_sizes[c]) {
            target_class = c;
            break;
        }
    }
...
void mk_pool_free(void *ptr)
{
    ...
    /* Return to class free list: O(1) */
    node->magic = MK_POOL_MAGIC_FREE;
    node->in_use = 0U;
    node->next_free = s_free_heads[c];
    s_free_heads[c] = node;

    if (s_stats.classes[c].allocated_blocks > 0) {
        s_stats.classes[c].allocated_blocks--;
    }
    if (s_stats.total_used_bytes >= s_block_sizes[c]) {
        s_stats.total_used_bytes -= s_block_sizes[c];
    }
}

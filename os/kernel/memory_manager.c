...
typedef struct mk_mem_block {
    uint32_t magic;
    uint32_t size;
    struct mk_mem_block *prev;
    struct mk_mem_block *next;
    uint32_t is_free;
    uint32_t padding;
} mk_mem_block_t;
...
void mk_memory_reset(void) { (void)mk_memory_init(); }

void *my_malloc(size_t size)
{
    if (!s_initialized && mk_memory_init() != MK_STATUS_OK) return NULL;
...
void my_free(void *ptr)
{
    ...
    if (ptr_val < arena_start + sizeof(mk_mem_block_t) || ptr_val >= arena_end || (ptr_val & (MK_ALIGNMENT - 1U)) != 0U) {
    ...
    if (block->prev != NULL && block->prev->is_free != 0U) {
        ...
        if (block->next != NULL) block->next->prev = prev_block;
        s_stats.free_bytes += sizeof(mk_mem_block_t);
    }
}

size_t mk_memory_used(void) { return s_stats.used_bytes; }
size_t mk_memory_free(void) { return s_stats.free_bytes; }
size_t mk_memory_peak(void) { return s_stats.peak_used_bytes; }
void mk_memory_get_stats(mk_mem_stats_t *out_stats) { if (out_stats != NULL) *out_stats = s_stats; }
